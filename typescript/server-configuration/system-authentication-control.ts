...
export async function systemAuthenticationControl(): Promise<void> {
    ...
    const authenticationConfiguration = await session.security.getSystemAuthenticationConfiguration();
    console.log(JSON.stringify(authenticationConfiguration, undefined, 4));
    ...
    const authenticationScript3 = session.security.authenticationScriptBuilder()
        .allowAnonymousConnections([])
        .removePrincipal('observer')
        .build();
    await session.security.updateAuthenticationStore(authenticationScript3);
