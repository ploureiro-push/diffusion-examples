...
export async function monitoringSessionPropertiesListener(): Promise<void> {
    ...
    const sessionEventStream: SessionEventStream = {
        onSessionEvent: (event: SessionEvent) => {
            if (event.type === SessionEventStreamEventType.STATE && event.state === SessionState.ACTIVE) {
            ...
            } else if (event.type === SessionEventStreamEventType.STATE && event.state === SessionState.CLOSED) {
                console.log(
                ...
                console.log(
                    `Session ${session.toString()} has been updated (${SessionEventStreamEventType[event.type]}) `
                    + `with the following properties:`
                );
                for (const key of Object.keys(event.properties)) {
                    console.log(`  ${key}: ${event.properties[key]}`);
