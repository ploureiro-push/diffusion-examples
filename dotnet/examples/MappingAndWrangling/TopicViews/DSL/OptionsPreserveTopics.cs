...
namespace PushTechnology.ClientInterface.Examples.MappingAndWrangling.TopicViews.DSL
{
    public sealed class OptionsPreserveTopics : Example
    {
        public override async Task Run(CancellationToken cancellationToken, string[] args)
        {
            ...
            var view1 = await session.TopicViews.CreateTopicViewAsync("topic_view_1", "map my/topic/path to views/preserved preserve topics", cancellationToken);
            WriteLine($"Topic View {view1.Name} has been created.");

            var view2 = await session.TopicViews.CreateTopicViewAsync("topic_view_2", "map my/topic/path to views/not_preserved", cancellationToken);
