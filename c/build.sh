...
function build_for_unix() {
    ...
    if $is_macos; then
        export SOURCE_DIR TARGET_DIR cmake_dir operating_system architecture is_macos CMAKE_BUILD_TYPE

        # Pass function definition and invoke it inside the new shell
        set -x
        arch "-$architecture" /bin/zsh --login -c "$(declare -f build_commands); source ~/.zshrc && source ~/.profile && source ~/.zprofile && set -a; SOURCE_DIR=${SOURCE_DIR}; is_macos=${is_macos}; TARGET_DIR=${TARGET_DIR}; cmake_dir=${cmake_dir}; operating_system=${operating_system}; architecture=${architecture}; CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}; build_commands" || exit 1
        set +x
    else
        build_commands
    fi
