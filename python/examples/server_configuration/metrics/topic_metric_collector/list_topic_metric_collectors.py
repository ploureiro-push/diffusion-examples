...
class ListTopicMetricCollectors(Example):
    async def run(
        ...
        async with sessions().principal(principal).credentials(
            ...
            builder = builder.group_by_topic_type(True).group_by_topic_view(
                True
            )
            ...
            builder = TopicMetricCollectorBuilder().export_to_prometheus(True)
            builder = builder.group_by_topic_type(False).group_by_topic_view(
                True
            )
            builder = builder.maximum_groups(250)
            collector = builder.create(
                "Topic Metric Collector 2", topic_selector
            )
            await session.metrics.put_topic_metric_collector(collector)
